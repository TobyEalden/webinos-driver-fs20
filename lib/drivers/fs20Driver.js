  ...
  function onPacketReceived(timestamp,packet) {
    ...
    if (deviceCode in deviceList) {
      ...
      if (adapter.applyTo(fhtInst) === 0) {
        callbackFunc('data', fhtInst.temperatureSensorServiceId, fhtInst.getMeasuredTemp().toFixed(1) );
        callbackFunc('data', fhtInst.valveSensorServiceId, fhtInst.getValvePosition().toFixed(1) );
      }
    } else {
        ...
        switch(cmd) {
            ...
            case 'start':
                ...
                for(var dev in liveDeviceMap.devices) {
                    if (liveDeviceMap.devices[dev].temperatureSensorServiceId === eId || liveDeviceMap.devices[dev].valveSensorServiceId === eId) {
                      var device = liveDeviceMap.devices[dev];
                      device.running = true;
                      setTimeout(function() {
                        console.log("---> initial temp. data is: " + device.getMeasuredTemp().toFixed(1));
                        callbackFunc('data', device.temperatureSensorServiceId, device.getMeasuredTemp().toFixed(1) );
                        console.log("---> initial valve data is: " + device.getValvePosition().toFixed(1));
                        callbackFunc('data', device.valveSensorServiceId, device.getValvePosition().toFixed(1) );
                      }, 2000);
                      break;
                    }
                }
